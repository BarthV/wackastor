...
// GET: list active reservations where current user is owner, filtered by requesterId
export const GET: RequestHandler = async ({ params, locals }) => {
	...
	const rows = await db
		...
		.where(
			and(
				eq(reservations.ownerId, locals.user.id),
				eq(reservations.requesterId, params.requesterId),
				activeReservation
			)
		);
